...
export default function CustomerCardModal({ initialKey, onClose, nav, onNavigate }: Props) {
  ...
  const dirty = useMemo(() => {
    ...
    const notesChanged = !!profile?.user && draft.notes !== (profile.user.staff_notes ?? '')
    if (!focused) return notesChanged
    return notesChanged
  ...
  async function save() {
    ...
    if (ok && profile?.user && draft.notes !== (profile.user.staff_notes ?? '')) {
