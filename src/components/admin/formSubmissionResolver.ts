...
export function resolveSubmitter(form: ResolverForm, submission: ResolverSubmission): ResolvedSubmitter {
  ...
  // Walk fields. Override > heuristic. First match per role wins.
  ...
  for (const field of form.fields_json) {
    ...
    else if (role === 'name_first' && !firstName) firstName = value
    else if (role === 'name_last' && !lastName) lastName = value
    else if (role === 'name' && !fullName) fullName = value
  }
