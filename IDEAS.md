...
## הרשמה של יותר מאדם אחד (זוג הורים)

**Status:** בקשה של יהב, 12.8.26. טרם נבנה.
**Source:** "אם זוג רוצה להירשם יחד לפתוח אופציה להרשמה של יותר מאחד, למשל זוג הורים לסדנת עזרה ראשונה"

### מה צריך לקרות
בטופס ההרשמה הציבורי, במוצרים שמסומנים לכך, תופיע בחירה של מספר משתתפים
(1 או 2) ושדה לשם המשתתף השני. ההרשמה נשמרת כשורה אחת עם רשימת משתתפים,
תופסת שני מקומות בתפוסת המחזור, והמחיר מוכפל.

### מה שצריך להחליט לפני שבונים
1. **התשלום.** לינקי Morning הם מחיר קבוע. הרשמה זוגית דורשת או לינק שני
   ייעודי לזוג בכל מוצר רלוונטי, או מעבר לסכום דינמי. זו ההחלטה הכי גדולה כאן
   ושווה לשאול את יהב לפני שכותבים קוד.
2. **לאילו מוצרים.** עזרה ראשונה בוודאות. מפגש אבות כנראה. עטופים ומגלים
   כנראה לא, כי הן סדנאות אם ותינוק. עדיף דגל פר מוצר בניהול ולא ניחוש.
3. **מה נשלח למשתתף השני.** האם הוא מקבל שאלון משלו, נכנס לקבוצת הוואטסאפ,
   ומקבל תזכורות. אם כן, צריך גם טלפון שלו ולא רק שם.

### הערה טכנית
`registration_leads` היא שורה לאדם. הכי נקי להוסיף `participants jsonb`
ו-`seats int default 1`, ולעדכן את הספירות שמזינות תפוסת מחזור כך שיסכמו
`seats` במקום לספור שורות. כל מקום שסופר הרשמות בניהול צריך לעבור על זה.
