...
//
//   CHROME_PATH=... node story.mjs [out.png]
//
// The bottom third is deliberately left empty. That is where the questions
// sticker goes when the story is posted, and anything drawn there would end
// up underneath it.
...
const out = process.argv[2] || resolve(dir, 'story-3-pages.png')
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  header { position: absolute; top: 100px; left: 0; right: 0; text-align: center; }
  header img { width: 320px; }
  h1 {
    font-family: "Mimo He", "Nunito", sans-serif; font-weight: 400;
    font-size: 96px; line-height: 1; color: #A35C3D; margin-top: 22px;
  }
  ...
  #gallery {
    position: absolute; top: 425px; left: 0; right: 0;
  ...
  #labels {
    position: absolute; top: 1305px; left: 0; right: 0;
  ...
  #ask {
    position: absolute; top: 1450px; left: 0; right: 0; text-align: center;
...
<div id="ask">
  יש לך שאלה על האפליקציה?
</div>

</body></html>`

writeFileSync(resolve(dir, '_story.html'), html)
const browser = await chromium.launch({ executablePath: process.env.CHROME_PATH || undefined })
const page = await (await browser.newContext({ viewport: { width: 1080, height: 1920 } })).newPage()
await page.goto('file://' + resolve(dir, '_story.html'))
