...
`prep.mjs` הוא השלב שמכין את הקול, במקום `timing.mjs`:

    node prep.mjs deck-yoman.json <תיקיית-הגלם> narration/yoman

הוא חותך את השקט מההתחלה ומהסוף, מיישר עוצמה בין הקלטות שנעשו
בישיבות שונות, וכותב לדק את משך השהייה של כל שקופית.

יהב לא תמיד מקליט משפט אחד לקובץ. לפעמים הוא קורא שניים־שלושה
ברצף ומסמן `13+14`. במקרה כזה `prep.mjs` מחפש את ההפסקה שלקח בין
המשפטים וחותך שם, כך שהתמונה מתחלפת על ההפסקה ואף מילה לא נחתכת.
מפת הקבוצות יושבת ב-`GROUPS` בראש הקובץ.

ביומן השמות של הקבצים לא הסתדרו: תשעה־עשר קבצים לעשרים ושניים
משפטים, והמספר הגבוה ביותר היה 20. חישוב אורך הדיבור בכל קובץ
הכריע — רק קבצים 16 ו-19 ארוכים מספיק כדי להכיל שני משפטים, ולכן
16 מדבר על שקופיות 16-17 ו-19 על 20-21. זו לא הערכה, זו האפשרות
היחידה שמסתדרת.

בהקלטה של המוצרים הוא קיצר: קובץ אחד לארבע שקופיות (2-5) עם
כחמש וחצי שניות דיבור בלבד. השקופיות האלה מתחלקות את הזמן לפי
אורך הכתובית, ושתיים מהן נשארות בלי משפט משלהן.

### חוב הדעיכה

כל מעבר `xfade` חופף לשכן שלו, כלומר קטע תופס `dur - FADE` על ציר
הזמן הסופי. `assemble` מוסיף את החוב הזה בחזרה לשקופית האחרונה של
כל צעד, אחרת השקופית על המסך קצרה מה-`hold` שביקשנו והמשפט שהוקלט
עבורה גולש אל התמונה הבאה. `verify.mjs` תופס בדיוק את זה.
