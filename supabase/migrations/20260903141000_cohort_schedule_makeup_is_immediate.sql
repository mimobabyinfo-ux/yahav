-- מוסיפה את makeup_is_immediate ל-get_my_cohort_schedule, כדי שהממשק יידע
-- אם ההשלמה הממתינה שלה היא על קבוצה שכבר "ידועה" (התשובה יכולה להגיע
-- מוקדם, ברגע שיתפנה מקום) או קבוצה שעוד לא נפתחה (התשובה רק סמוך למועד).
...
as $$
  ...
  select mine.lead_id, mine.workshop_id, w.title, mine.cohort_id, c.start_date,
    ...
    case when q.id is not null then public.cohort_enrollment_settled(tm.id) end,
